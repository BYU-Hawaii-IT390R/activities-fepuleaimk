...
# ══════════════════════════════════════════════════════════════════════════
# Task 4: Local user accounts (win-users)
# ══════════════════════════════════════════════════════════════════════════

def win_users():
    print("\n👥 Local user accounts")
    try:
        out = subprocess.check_output(["net", "user"], text=True, stderr=subprocess.STDOUT)
        lines = out.splitlines()
        start = next(i for i, line in enumerate(lines) if "----" in line) + 1
        end = next(i for i, line in enumerate(lines[start:], start=start) if not line.strip())
        users = []
        for line in lines[start:end]:
            users += line.split()
        for user in users:
            try:
                details = subprocess.check_output(["net", "user", user], text=True)
                enabled = "Yes" if "Account active               Yes" in details else "No"
                last_logon = re.search(r"Last logon\s+(.+)", details)
                logon_str = last_logon.group(1).strip() if last_logon else "N/A"
                print(f"{user:<20} Enabled: {enabled:<3} Last logon: {logon_str}")
            except subprocess.CalledProcessError:
                print(f"{user:<20} [Error reading details]")
    except Exception as e:
        print(f"❌ Failed to list users: {e}")
    print()

# ══════════════════════════════════════════════════════════════════════════
# Task 5: Disk usage stats (win-disk)
# ══════════════════════════════════════════════════════════════════════════
...
def win_disk():
    ...
    for letter in ascii_uppercase:
        drive = f"{letter}:\\"
        if windll.kernel32.GetDriveTypeW(f"{drive}") < 2:
            continue  # Skip unknown/non-existent drives
        try:
            total, used, free = shutil.disk_usage(drive)
            print(f"{drive:<6} Total: {total//2**30:>4} GB | Free: {free//2**30:>4} GB")
        except FileNotFoundError:
            continue
    print()
...
def main():
    ...
    p.add_argument("--task", required=True,
                  choices=["win-events", "win-pkgs", "win-services", "win-users", "win-disk"],
